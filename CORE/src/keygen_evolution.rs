...
//!
//! Ecuación fundamental DEL DOCUMENTO DE FUNDAMENTACIÓN:
//! z(n) = φ · z(n-1) · (1 - z(n-1)/196884)
//! con z(0) = 196883/196884 ≈ 0.99999492 (materia potencial inicial)
//!
//! IMPORTANTE: z está escalado entre 0 y 1, donde 1 representa 196884
...
pub struct KeygenEvolution {
    /// Valor actual del keygen z(n) ∈ [INITIAL_KEYGEN, 1.0]
    current_keygen: f64,
    /// Iteración actual n
    iteration: u64,
...
impl KeygenEvolution {
    ...
    pub fn evolve(&mut self) -> f64 {
        ...
        // ECUACIÓN CORREGIDA SEGÚN DOCUMENTO DE FUNDAMENTACIÓN
        // z(n) = φ · z(n-1) · (1 - z(n-1)/196884)
        // Nota: z está escalado entre 0 y 1, donde 1 = 196884
        let z_scaled = z_prev * MONSTER_DIM;  // Convertir a escala Monster
        let z_next_scaled = PHI * z_scaled * (1.0 - z_scaled / MONSTER_DIM);
        let z_next = z_next_scaled / MONSTER_DIM;  // Volver a escala [0,1]
        ...
        // Limitar al rango [INITIAL_KEYGEN, 1.0]
    ...
    pub fn growth_acceleration(&self) -> f64 {
        ...
        let last_rate = rates[rates.len() - 1];
...
mod tests {
    ...
    fn test_single_evolution() {
        ...
        println!("Keygen inicial: {}", initial);
        
        let next = system.evolve();
        println!("Keygen después de 1 evolución: {}", next);
        
        // Debe crecer ligeramente (φ-resonante)
        assert!(next > initial, "Keygen debe crecer: {} > {}", next, initial);
        assert!(next < 1.0, "Keygen debe ser < 1.0: {}", next);
    ...
    fn test_multiple_evolutions() {
        ...
        println!("Evolución de 10 pasos: {:?}", results);
    }

    #[test]
    fn test_evolution_to_threshold() {
        let mut system = KeygenEvolution::new(None);
        let threshold = 0.999999;  // Umbral muy cercano a 1.0
    ...
    fn test_growth_metrics() {
        ...
        // La tasa debe ser positiva pero pequeña
        assert!(rate > 0.0);
        assert!(rate < 0.1);  // Crecimiento lento
    }
    ...
    fn test_projection() {
        ...
        println!("Proyección 20 pasos (últimos 5): {:?}", &projection[15..]);
    }
    ...
    fn test_stats_generation() {
        ...
        assert!(stats.distance_to_monster > 0.0);
    }
    ...
    fn test_batch_evolution() {
        ...
        for (i, evolution) in results.iter().enumerate() {
            ...
            println!("Batch {}: valores: {:?}", i, evolution);
        }
    }
    ...
    fn test_saturation_detection() {
        // Sistema que evolucionó mucho
        let mut system = KeygenEvolution::new(Some(0.9999999));
        system.evolve_steps(5);
