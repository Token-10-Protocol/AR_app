...
mod tests {
    ...
    fn test_propiedad_amor_phi_inv() {
        ...
        // Primera verificación: dentro de tolerancia alta
        assert_relative_eq!(
            tau_resultado,
            PHI_INV,
            epsilon = TOLERANCIA_ALTA,
            message = "Fallo en tolerancia alta (1e-6)"
        );
        
        // Intentar verificación con tolerancia media
        let diferencia = (tau_resultado - PHI_INV).abs();
        if diferencia < TOLERANCIA_MEDIA {
            ...
            panic!("❌ Diferencia demasiado grande: {:.2e} > {:.0e}", 
